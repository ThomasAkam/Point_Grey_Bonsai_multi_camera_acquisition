...
# (c) Thomas Akam 2019.  Licenced under the GNU General Public License v3.
...
for box, subject in subjects.items():
    pinstate_file_path = os.path.join(data_dir, subject + '_pinstate_' + datetime_str + '.csv')
    command += (' -p:Box{}.Index={}'.format(box, camera_IDs[box]) + 
    	        ' -p:Box{}.StartTime=0'.format(box) + 
